...
class ChatUI {
    ...
    getPromptKeys(language, engine) {
        ...
        if (isTestChecked) {
            if (this.isJavaSelenium(lang, eng) || this.isCSharpSelenium(lang, eng) || this.isPythonSelenium(lang, eng)) {
                promptKeys.push('TEST_SCRIPT_ONLY');
            } else if (this.isTypeScriptPlaywright(lang, eng)) {
                promptKeys.push('PLAYWRIGHT_TS_TEST_SCRIPT_ONLY');
            } else {
                this.addUnsupportedLanguageMessage(lang, eng);
            }
        }
